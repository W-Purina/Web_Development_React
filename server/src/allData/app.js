...
async function clearDatabase(collection) {
  try {
    ...
    console.error(
      "Error occurred while removing all documents from the collection:",
      error
    );
  }
}
...
async function addUsers_test() {
  ...
  for (let userInfo of randomData.users) {
    ...
    try {
      ...
      console.error(
        "Error occurred while adding new user to the database:",
        error
      );
    }
  }
...
async function addGroups_test() {
  ...
  for (let groupData of randomData.groups) {
    try {
      ...
      console.error(
        "Error occurred while adding new group to the database:",
        error
      );
    }
  }
...
async function addOrders_test() {
  ...
  for (let orderData of randomData.orders) {
    try {
      ...
      console.error(
        "Error occurred while adding new order to the database:",
        error
      );
    }
  }
...
async function addGroups(groupData) {
  try {
    ...
    const createdByUser = await User.findById(
      new mongoose.Types.ObjectId(groupData.createdBy)
    );
...
async function updateGroup(groupId, group) {
  try {
    ...
    // 更新小组
    const updatedGroup = await Group.findOneAndUpdate({ _id: groupId }, group);
...
async function getGroupById(groupId) {
  try {
    ...
    console.error(
      "Error occurred while getting group from the database:",
      error
    );
    return null;
  }
...
async function deleteGroupById(groupId) {
  try {
    ...
    console.error(
      `Error occurred while deleting the group with ID ${groupId}:`,
      error
    );
    return null;
  }
...
async function getRecentPurchasesByGroupId(groupid) {
  try {
    ...
    const productNames = recentPurchases.map(
